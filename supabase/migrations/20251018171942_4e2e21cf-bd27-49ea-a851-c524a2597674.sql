-- Add proportional_fixed_cost column to financial metrics view
DROP VIEW IF EXISTS rank_rent_financial_metrics;

CREATE VIEW rank_rent_financial_metrics
WITH (security_invoker=true)
AS
WITH site_page_counts AS (
  SELECT 
    site_id,
    COUNT(*) as rented_pages_count
...
SELECT 
  ...
  -- NEW: Proportional fixed cost per page
  (COALESCE(fc.monthly_fixed_costs, 0) / GREATEST(spc.rented_pages_count, 1)) as proportional_fixed_cost,
  ...
  -- Use proportional cost in calculations
  (p.monthly_rent_value 
   - (COALESCE(fc.cost_per_conversion, 0) * COUNT(DISTINCT conv.id)) 
   - (COALESCE(fc.monthly_fixed_costs, 0) / GREATEST(spc.rented_pages_count, 1))) as monthly_profit,
  
  CASE 
    WHEN (COALESCE(fc.cost_per_conversion, 0) * COUNT(DISTINCT conv.id) 
          + (COALESCE(fc.monthly_fixed_costs, 0) / GREATEST(spc.rented_pages_count, 1))
          + COALESCE(fc.acquisition_cost, 0)) > 0
    THEN ((p.monthly_rent_value 
           - (COALESCE(fc.cost_per_conversion, 0) * COUNT(DISTINCT conv.id)) 
           - (COALESCE(fc.monthly_fixed_costs, 0) / GREATEST(spc.rented_pages_count, 1))) / 
          (COALESCE(fc.cost_per_conversion, 0) * COUNT(DISTINCT conv.id) 
           + (COALESCE(fc.monthly_fixed_costs, 0) / GREATEST(spc.rented_pages_count, 1))
           + COALESCE(fc.acquisition_cost, 0)) * 100)
    ELSE 0
  END as roi_percentage,
  
  CASE 
    WHEN p.monthly_rent_value > 0
    THEN ((COALESCE(fc.cost_per_conversion, 0) * COUNT(DISTINCT conv.id) 
           + (COALESCE(fc.monthly_fixed_costs, 0) / GREATEST(spc.rented_pages_count, 1))) 
          / p.monthly_rent_value * 100)
    ELSE 0
...
JOIN site_page_counts spc ON spc.site_id = p.site_id
...
GROUP BY 
  ...
  s.user_id, spc.rented_pages_count;
-- 1. Remover trigger e função antiga create_default_pipeline_stages (PRIMEIRO)
...
-- 2. Atualizar handle_new_user() para criar estágios do pipeline junto com profile e role
...
-- 3. Criar estágios para usuários existentes que não têm
INSERT INTO crm_pipeline_stages (user_id, stage_key, label, color, display_order, is_system, is_active)
SELECT 
  p.id as user_id,
  stages.stage_key,
...
) AS stages(stage_key, label, color, display_order, is_system, is_active)
WHERE NOT EXISTS (
